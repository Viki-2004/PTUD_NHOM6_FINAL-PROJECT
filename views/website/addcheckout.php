<?php
    ...
    if ($insert_orders) {
        ...
        foreach ($_SESSION['cart'] as $key => $value) {
            ...
            $price = $value['price'];
            $thanhtien = $order_quantity*$price;
            $totalbill +=$thanhtien;
            $insert_order_details = "INSERT INTO order_details (sku, order_id,user_id, order_quantity, price, total) VALUES ('".$sku."','".$order_id."','".$user_id."','".$order_quantity."','".$price."','".$thanhtien."')";
            mysqli_query($conn, $insert_order_details);
        }
        $insert_totalbill = "UPDATE orders SET total_bill = $totalbill WHERE user_id = $user_id ORDER BY created_at DESC LIMIT 1" ;
        $totalbill_query = mysqli_query ($conn, $insert_totalbill);
    }
    unset($_SESSION['cart']);
